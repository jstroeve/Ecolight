...
=======
set_of_siconc_models = set(sic_files)
set_of_sithick_models = set(sit_files)
set_of_swu_models = set(swu_files)
set_of_swd_models = set(swd_files)
set_of_snd_models = set(snd_files)
# sic_files_new=[]
# for file in sic_files:
#     set_of_siconc_models = set(file.replace("siconc",""))
#     sic_files_new.append(set_of_siconc_models)
# for file in sit_files:
#     set_of_sithick_models = set(file.replace("sithick",""))
# for file in swu_files:
#     set_of_swu_models = set(file.replace("siflswutop",""))
# for file in swd_files:
#     set_of_swd_models = set(file.replace("siflswdtop",""))
# for file in snd_files:
#     set_of_snd_models = set(file.replace("sisnthick",""))

#note this doesn't work since of course th filenames have the arrays in them, so we need to remove the variable name at the start of each set of models
#in order to find the set that matches in terms of model name and ensemble member
models_with_all_variables = (set_of_siconc_models & set_of_sithick_models & set_of_snd_models & set_of_swu_models & set_of_swd_models) # Find intersection of sets

# new_files=[]
# files=filenames['Model Name']
# for model in files:
#     print(modelnew_)
#     if ((model in sic_files) & (model in sit_files) & (model in snd_files) & (model in swu_files) & (model in swd_files)):
#         new_files.append(model)
...
#### Get latitude, longitude from CMIP6 data file
#assuming that all the lat/lons are the same as this is the sea ice grid
inpath=datapath
files=os.listdir(inpath)#inpath
ncf=nc.Dataset(inpath+files[0])   #just use the first file from the siconc to set the latitude/longintude
try:
...
#loop over all the variables per model/ensemble
output=[]
for ncvar in ncvarlist: #loop through each variable name
    files=datapath+ncvar+models_with_all_variables
    ncparts=[]
    for f in files: #for each file that matches for all variables, load the array
        ncf=nc.Dataset(f)
        ncstart=int(ncf['time'].units.strip('days since ')[0:4])
        times=ncf['time'][:].data
        #load each part of the data for this variable
...
#     mp.drawcoastlines()
 ...
 # We have 2 rows and 2 columns in our plot grid
>>>>>>> c2d2663a2f9d985d057c010c1ec4b2045c70c7c9
